...
|:--|:--|
| **`/cresearch <topic>`** | Web research from many angles — overview, statistics, arXiv/PubMed papers, recent news — compiled into a self-contained HTML report. The workhorse. |
...
- **Key findings** — the substance, in sections that fit the topic, with `<table>`s for numbers and blockquotes for the quotes worth pulling
...
- **Primary sources over blog rehashes.** The skills fetch full pages only for sources that genuinely add depth — seminal papers, primary data — rather than scraping every search hit.
